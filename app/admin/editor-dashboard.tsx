...
const copy = {
  zh: {
    ...
    title: '标题', titlePlaceholder: '输入文章标题',
    subtitle: '副标题（可选）', subtitlePlaceholder: '输入副标题', subtitleHelp: '显示在文章标题下方；不填写则不显示。',
    summary: '简介', summaryHelp: '用一两句话告诉读者这篇文章讲什么。',
    author: '作者', date: '发布时间', category: '分类', categoryPlaceholder: '例如：自然',
    tags: '标签', tagsHelp: '多个标签用逗号隔开。发布后会自动进入站内分类与标签列表。', tagsPlaceholder: '自然，文化，思想',
    cover: '封面图', uploadCover: '上传封面', replaceCover: '更换封面', removeCover: '移除封面',
    ...
    saveDraft: '存为草稿', publish: '发布', delete: '删除', draftStatus: '草稿', publishedStatus: '已发布',
  ...
  en: {
    ...
    title: 'Title', titlePlaceholder: 'Type the article title',
    subtitle: 'Subtitle (optional)', subtitlePlaceholder: 'Type the subtitle', subtitleHelp: 'Shown directly below the article title. Leave blank to hide it.',
    summary: 'Summary', summaryHelp: 'Describe what the article is about in one or two sentences.',
    author: 'Author', date: 'Publish date', category: 'Category', categoryPlaceholder: 'e.g. Nature',
    ...
    cover: 'Cover image', uploadCover: 'Upload cover', replaceCover: 'Replace cover', removeCover: 'Remove cover',
    ...
    saveDraft: 'Save draft', publish: 'Publish', delete: 'Delete', draftStatus: 'Draft', publishedStatus: 'Published',
...
export function EditorDashboard() {
  ...
  async function uploadInline(event: ChangeEvent<HTMLInputElement>) {
    ...
    setBusy(true);
    try {
      editor.chain().focus().setImage({ src: await upload(file), alt: '' }).run();
        ...
        <div className="field-grid">
          ...
          <div className="field full">
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              {draft.coverImage && <button type="button" className="text-button" onClick={() => setDraft((current) => ({ ...current, coverImage: '', coverAlt: '' }))}>{t.removeCover}</button>}
            </div>
          </div>
